...
template <typename E> inline int findpivot(E A[], int i, int j) {
  return (i + j) / 2;
}
...
template <typename E, typename Comp> void qsort(E array[], int i, int j) {
  if ((j - i) <= THRESHOLD)
    return; // Don't sort short list
...
template <typename E, typename Comp> void sort(E *array, int n) {
...
int main(int argc, char **argv) { return sortmain<minintCompare>(argc, argv); }
